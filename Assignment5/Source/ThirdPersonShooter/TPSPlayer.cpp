...
void ATPSPlayer::StartZoom_Implementation()
{
	ATPSCharacter::StartZoom_Implementation();
	targetFOV = zoomedFOV;
}

void ATPSPlayer::EndZoom_Implementation()
{
	ATPSCharacter::EndZoom_Implementation();
	targetFOV = defaultFOV;
}

void ATPSPlayer::StartFire_Implementation()
{
	if (currentPlayerTeam != PlayerTeam::None) {
		ATPSCharacter::StartFire_Implementation();
	}
}

void ATPSPlayer::EndFire_Implementation()
{
	if (currentPlayerTeam != PlayerTeam::None) {
		ATPSCharacter::EndFire_Implementation();
	}
}
...
void ATPSPlayer::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
	...
	DOREPLIFETIME(ATPSPlayer, targetFOV);
	DOREPLIFETIME(ATPSPlayer, currentPlayerTeam);
}
...
void ATPSPlayer::Tick(float DeltaTime)
{
	...
	CameraComp->SetFieldOfView(FMath::FInterpTo(CameraComp->FieldOfView, targetFOV, DeltaTime, zoomSpeed));
}
...
void ATPSPlayer::RandomFunctionOver_Implementation()
{
	ATPSGameMode * mymode = Cast<ATPSGameMode>(GetWorld()->GetAuthGameMode());
	mymode->PlayerNumber++;

	currentPlayerTeam = mymode->PlayerNumber % 2 == 0 ? PlayerTeam::TeamA : PlayerTeam::TeamB;
}

bool ATPSPlayer::RandomFunctionOver_Validate() {
	return true;
}
