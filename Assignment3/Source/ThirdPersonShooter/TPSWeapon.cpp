...
void ATPSWeapon::Fire()
{
	...
	if (MyOwner && ammoCount > 0 && MyOwner->GetCurrentWeaponState() == WeaponState::Shooting)
	{
		...
		if (GetWorld()->LineTraceSingleByChannel(Hit, EyeLoc, LineEnd, WeaponTraceChannel, QueryParams))
		{
			...
			if (DamagePowerUpEnabled) {
				GEngine->AddOnScreenDebugMessage(-1, 2, FColor::Orange, "Pew Pew -> Boom Boom -> Supermultiplier!");
				DamageToApply *= DamagePowerUp;
			}
...
void ATPSWeapon::MakePoweful() {

	MeshComp->CreateAndSetMaterialInstanceDynamicFromMaterial(0, BlinkMaterial);
	DamagePowerUpEnabled = true;
}
void ATPSWeapon::BackToNormal() {
	MeshComp->CreateAndSetMaterialInstanceDynamicFromMaterial(0, OriginalMaterial);
	DamagePowerUpEnabled = false;
}
